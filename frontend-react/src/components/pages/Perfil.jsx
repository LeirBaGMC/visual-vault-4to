...
const Perfil = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (mainMenuRef.current && !mainMenuRef.current.contains(event.target)) setIsMainMenuOpen(false);
            if (pillMenuRef.current && !pillMenuRef.current.contains(event.target)) setIsPillMenuOpen(false);
        };
    ...
    const toggleLike = (e, pin) => {
        ...
        setLikedPins(prev => {
            ...
            if (newLikes.has(pinId)) {
                ...
                setCategoryWeights(w => ({ ...w, [cat]: Math.max((w[cat] || 0) - 1, 0) }));
            } else {
                newLikes.add(pinId);
    ...
    const categoriasUnicas = ["Todas", "Para Ti", ...new Set(pins.map(pin => pin.category).filter(Boolean))];
        ...
        <div className="min-h-screen bg-[#090B0E] p-4 md:p-8 text-white relative">
            ...
            {toastMsg && (
                <div className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] bg-zinc-800 text-white px-6 py-3 rounded-full shadow-2xl border border-zinc-700 animate-fade-in-up flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold tracking-widest uppercase">{toastMsg}</span>
                </div>
            )}
            ...
            {/* HEADER EXPANDIDO */}
            <div className={`fixed z-50 top-4 left-4 right-4 md:left-8 md:right-8 bg-zinc-900/80 backdrop-blur-xl border border-zinc-800 rounded-2xl p-4 shadow-2xl flex flex-col md:flex-row items-center justify-between transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]
            ...
            >
                <div className="flex gap-3 w-full md:w-1/2">
                    <div className="relative w-full">
                        ...
                        <input 
                            ...
                            className="w-full bg-zinc-950 border border-zinc-700 text-white text-sm pl-10 pr-4 py-3 rounded-xl focus:outline-none focus:border-gray-400 transition-colors"
                        />
                    </div>
                    <button onClick={() => alert("Módulo de Ingesta S3")} className="bg-white hover:bg-gray-200 text-black px-4 py-3 rounded-xl shadow-lg flex items-center justify-center flex-shrink-0 font-bold text-sm tracking-wider uppercase transition-colors">
                        <svg className="w-5 h-5 md:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" /></svg>
                ...
                <div className="flex gap-4 w-full md:w-auto items-center justify-between md:justify-end mt-4 md:mt-0">
                    ...
                    <div className="relative flex-shrink-0" ref={mainMenuRef}>
                        <button 
                            onClick={(e) => { e.stopPropagation(); setIsMainMenuOpen(!isMainMenuOpen); }} 
                        ...
                        {isMainMenuOpen && (
                            <div className="absolute top-[130%] right-0 w-72 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-[0_15px_50px_rgba(0,0,0,0.9)] overflow-hidden z-[100] animate-fade-in-up">
                                <div className="p-4 border-b border-zinc-800">
                                    <p className="text-xs text-zinc-500 uppercase tracking-widest mb-3">Identidad de Sesión</p>
                                    <div className="flex items-center gap-3 bg-zinc-800/50 p-2 rounded-xl">
                                        <div className="w-10 h-10 bg-zinc-700 rounded-full flex items-center justify-center font-bold text-gray-300">MC</div>
                                        <div>
                                            <p className="text-sm font-bold text-white">Gabriel Minda</p>
                                            <p className="text-xs text-zinc-400">Personal</p>
                                        </div>
                                    </div>
                                </div>
                                <div className="p-2">
                                    <button className="w-full text-left px-4 py-3 text-sm text-zinc-300 hover:bg-zinc-800 rounded-lg transition-colors">Añadir cuenta</button>
                                    <button onClick={() => navigate('/')} className="w-full text-left px-4 py-3 text-sm text-zinc-300 hover:bg-zinc-800 rounded-lg transition-colors">Volver al Inicio</button>
                                    <button onClick={() => navigate('/login')} className="w-full text-left px-4 py-3 text-sm text-red-400 hover:bg-red-950/50 rounded-lg transition-colors">Cerrar sesión</button>
                                </div>
                            </div>
                        )}
                    </div>
            ...
            <div className={`fixed z-50 top-4 right-4 md:right-8 bg-zinc-900/90 backdrop-blur-xl border border-zinc-800 shadow-2xl rounded-full p-1.5 flex items-center gap-2 transition-all duration-400 ease-[cubic-bezier(0.34,1.56,0.64,1)]
                ${isHeaderCollapsed ? 'opacity-100 scale-100 pointer-events-auto' : 'opacity-0 scale-75 pointer-events-none'}`}
            >
                <button onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); setIsHeaderCollapsed(false); }} className="bg-transparent hover:bg-zinc-800 text-white p-2.5 rounded-full transition-colors group">
                ...
                <button onClick={() => alert("Módulo de Ingesta S3")} className="bg-zinc-800 hover:bg-zinc-700 text-white p-2.5 rounded-full transition-colors">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4" /></svg>
                </button>
                
                <div className="relative" ref={pillMenuRef}>
                    <button onClick={(e) => { e.stopPropagation(); setIsPillMenuOpen(!isPillMenuOpen); }} className="bg-zinc-700 text-white w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold font-display border border-zinc-600 hover:border-zinc-400 transition-all active:scale-95">
                        MC
                    </button>
                    {isPillMenuOpen && (
                        <div className="absolute top-[130%] right-0 w-72 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-[0_15px_50px_rgba(0,0,0,0.9)] overflow-hidden z-[100] animate-fade-in-up">
                            <div className="p-4 border-b border-zinc-800">
                                <p className="text-xs text-zinc-400 uppercase tracking-widest mb-3">Identidad de Sesión</p>
                                <div className="flex items-center gap-3 bg-zinc-800/50 p-2 rounded-xl">
                                    <div className="w-10 h-10 bg-zinc-700 rounded-full flex items-center justify-center font-bold">MC</div>
                                    <div>
                                        <p className="text-sm font-bold text-white">Gabriel Minda</p>
                                        <p className="text-xs text-zinc-400">Personal</p>
                                    </div>
                ...
                {filteredPins.length === 0 ? (
                    ...
                    <div className={`grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[220px] md:auto-rows-[280px] grid-flow-dense group ${isScrolling ? 'pointer-events-none' : ''}`}>
                        {filteredPins.map((pin, index) => {
                            ...
                            return (
                                <div key={pin.id} onClick={() => editingPinId !== pin.id && navigate(`/pin/${pin.id}`)} className={`relative rounded-sm overflow-hidden shadow-lg cursor-pointer flex flex-col justify-end p-6 group/card animate-fade-in-up ${patternClass}`} style={{ animationDelay: `${index * 0.05}s` }}>
                                    
                                    {/* 1. IMAGEN */}
                                    <div className="absolute inset-0 bg-cover bg-center transition-all duration-500 delay-0 group-hover:delay-500 group-hover:blur-[12px] group-hover:brightness-40 group-hover/card:!blur-none group-hover/card:!brightness-100 group-hover/card:!delay-0 group-hover/card:scale-[1.03]" style={{ backgroundImage: `url(${pin.image_url})` }} />
                                    ...
                                    {/* 3. ICONOS (AHORA VUELVEN A SER INVISIBLES HASTA EL HOVER MEDIANTE 'opacity-0 group-hover/card:opacity-100') */}
                                    ...
                                    <div className="z-10 w-full relative pointer-events-none opacity-0 group-hover/card:opacity-100 transition-all duration-300 translate-y-4 group-hover/card:translate-y-0">
                                        ...
                                        {isAdminMode && editingPinId !== pin.id && (
                                            <div className="flex gap-2 mt-3 z-30 relative pointer-events-auto" onClick={(e) => e.stopPropagation()}>
                                                <button onClick={() => { setEditingPinId(pin.id); setNewTitle(pin.title); }} className="bg-zinc-800/90 text-[10px] uppercase font-bold px-3 py-1 rounded-sm text-zinc-300 hover:text-white">Editar</button>
                                                <button onClick={(e) => eliminarPin(pin.id, e)} className="bg-red-950/90 text-[10px] uppercase font-bold px-3 py-1 rounded-sm text-red-400 hover:text-white">Borrar</button>
                                            </div>
                                        )}
                                    </div>
                                </div>
